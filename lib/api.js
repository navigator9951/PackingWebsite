...
/**
 * Checks if a store is editable
 * @param {string} storeId - The store ID
 * @returns {Promise<boolean>} - Whether the store is editable
 */
async function isStoreEditable(storeId) {
  try {
    const response = await fetch(`/api/store/${storeId}/is_editable`);
    if (!response.ok) {
      throw new Error(`Failed to check store editability: ${response.status}`);
    }
    const data = await response.json();
    return !!data.editable;
  } catch (error) {
    console.error("Error checking store editability:", error);
